...
import { fetchCheckingGuideStatus } from "../../../redux/store/checkingGuideStatusSlice";
...
export default function GuideDashboardHome() {
  ...
  const { guideBooking } = useSelector((state) => state.guideBooking);
  const dispatch = useDispatch();
  const navigate = useNavigate();

  useEffect(() => {
    dispatch(fetchAppUser());
    dispatch(fetchCheckingGuideStatus());
  }, [dispatch]);
  ...
  const handleAccept = () => {
    touristGuideAppAPI.get("/booking/guideBooking/confirm", {
      params: { id: guideBooking.bookingId }
    }).then((res) => {
          ...
          <div className="w-full">
            ...
            {notificationShow && (<div
              ...
              <div className="2xl:w-3/12 bg-gray-50 h-screen overflow-y-auto p-8 absolute right-0">
                ...
                {guideBooking // 👈 null and undefined check
                  && Object.keys(guideBooking).length !== 0
                  && Object.getPrototypeOf(guideBooking) === Object.prototype && (
                      ...
                      <div className="relative py-8 px-5 md:px-10 bg-white shadow-md rounded border border-gray-400">
                        ...
                        <div className="flex mb-5">
                          <div className="w-1/2 h-12 mr-2">
                            ...
                            <input id="name"
                                   value={guideBooking?.checkInDate?.split(".").join("-")}
                                   type="date"
                                   disabled={true}
                          ...
                          <div className="w-1/2 h-12">
                            ...
                            <input id="name"
                                   value={guideBooking?.checkOutDate?.split(".").join("-")}
                                   type="date"
                                   disabled={true}
                                   className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                                   placeholder="James" />
                        ...
                        <div className="relative mb-3 mt-2">
                          <input id="email2"
                                 ...
                                 disabled={true}
                                 value={guideBooking?.noOfMembers} />
                        </div>
                        ...
                        <div className="flex mb-5">
                          ...
                          <div className="w-1/2 h-12">
                            ...
                            <input id="name"
                                   value={guideBooking?.fullPayment}
                                   type="text"
                                   disabled={true}
                                   className="mb-5 mt-2 text-gray-600 focus:outline-none focus:border focus:border-indigo-700 font-normal w-full h-10 flex items-center pl-3 text-sm border-gray-300 rounded border"
                                   placeholder="James" />
